...
async function extractSigAlg(i: number): Promise<void> {
  try {
    const { stdout } = await execAsync(`openssl x509 -inform PEM -text -in certificates/certificate_${i}.pem`);
...
async function main() {
  ...
  fs.writeFileSync("signature_algorithms.json", JSON.stringify(obj));
}
