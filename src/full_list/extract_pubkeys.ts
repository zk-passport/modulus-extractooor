...
async function main() {
  ...
  fs.writeFileSync('publicKeysParsed.json', JSON.stringify(filteredPublicKeysParsed, null, 2));
  console.log("publicKeysParsed.json written!")
}

async function extractModulus(i: number): Promise<void> {
  try {
    const certTextres = await execAsync(`openssl x509 -text -in certificates/certificate_${i}.pem`);
...
// const pubkeyRes = await execAsync(`openssl x509 -pubkey -in openssl/certificate_${i}.pem`);
// if (pubkeyRes.stderr) {
